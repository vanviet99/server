...
const authController = {
  registerUser: async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Tên người dùng đã tồn tại', error });
    }
  },
  ...
  loginUser: async (req, res) => {
    try {
      ...
      const token = authController.generateToken({ userId: user._id, username: user.username,
        email: user.email,role:user.role })
        let user_refreshToken = await userModal.updateOne({username:user.username},{refreshToken:token.refreshToken})
    ...
    posttoken:async (req,res)=>{
        ...
        try {
            ...
            const token = authController.generateToken({ userId: user._id, username: user.username,
                email: user.email, role:user.role}) 
                const user_refreshToken = await userModal.updateOne({username:user.username},{refreshToken:token.refreshToken})
